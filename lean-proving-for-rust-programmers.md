...
<!-- class: section -->
<div class="kicker">Lean Development</div>

# Proof development in Lean

Lean is a **dependently-typed functional language**, with a trustworthy proof-checker.
...
<!-- class: section -->
<div class="kicker">Rust → Lean</div>
...
## A theorem prover with a small trusted computing base (TCB)
...
The payoff: easy proofs over high-level code.
...
<!-- class: dense -->
# Milhouse: verification feedback loop

The proof effort did more than certify existing code:

- packed leaf verification exposed a bug in updates through zero nodes
- `DenseTree` invariants made implicit structural assumptions explicit
- an overly permissive `bulk_update` operation was identified as unsound and removed
- `intra_rebase` was strengthened so subtree sharing preserves correct lengths
...
- AI puts formal verification within reach for an increasing number of projects!
- Tooling is still quite immature, so we shouldn't get **too** ambitious.
- Focus on correct **specifications**, let Lean take care of the proof checking.
- **Next**: finish Milhouse verification, verify some components of Lighthouse with Aeneas.
...
In Rust, `Option::and_then` is exactly `bind` with `m = Option`:
